...
$Delegate = foreach ($mb in $mbx) {Get-MailboxFolderPermission ($mb.Alias + ":\التقويم") | Select-Object Identity, @{name='AccessRights';Expression={$_.AccessRights -join ","}}, FolderName, IsValid, ObjectState, User}
