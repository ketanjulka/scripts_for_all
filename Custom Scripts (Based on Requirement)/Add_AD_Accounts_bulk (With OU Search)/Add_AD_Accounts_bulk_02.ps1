...
#Log File Path
$logfile = "C:\Users\Administrator\Desktop\AD_Scripts\Logs\" + "UsersNotCreated_02_" +$(Get-Date -Format dd-MM-yyyy_hh.mm.ss)+ ".log"
        ...
        try{
            ...
            $usr_valid = Get-ADUser -Filter 'SamAccountName -eq $usr_sam'
                ...
                if($usr_valid -eq $null)
                {
                    ...
                    New-ADUser -GivenName $usr.GivenName -Surname $usr.Surname -DisplayName $usr.DisplayName -SamAccountName $usr.SamAccountName -UserPrincipalName $set_upn -Path $OUSpt[1] -AccountPassword $password -City $usr.City -Company $usr.Company -Country $usr.Country -Department $usr.Department -Description $usr.Description -Division $usr.Division -EmployeeID $usr.EmployeeID -EmployeeNumber $usr.EmployeeNumber -Fax $usr.Fax -HomePage $usr.HomePage -HomePhone $usr.HomePhone -MobilePhone $usr.MobilePhone -OfficePhone $usr.OfficePhone -Initials $usr.Initials -Name $usr.Name -Office $usr.Office -Organization $usr.Organization -OtherName $usr.OtherName -POBox $usr.POBox -PostalCode $usr.PostalCode -State $usr.State -StreetAddress $usr.StreetAddress -Title $usr.Title -ChangePasswordAtLogon:$true -ErrorAction Stop
